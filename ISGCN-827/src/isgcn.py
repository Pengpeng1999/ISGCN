...
from isignedsageconvolution import SignedSAGEConvolutionBase, SignedSAGEConvolution_tm_1
from isignedsageconvolution import ListModule
...
class SignedGraphConvolutionalNetwork(torch.nn.Module):
    ...
    def setup_layers(self):
        ...
        self.m = self.args.m
        self.times = self.layers//self.args.m    # 网络的周期数
        self.r = self.layers % self.args.m  # 最后一层的数量
        ...
        self.negative_base_aggregator = SignedSAGEConvolutionBase(self.X.shape[1]*2,
                                                                  self.neurons[0]).to(self.device)
        self.list_layer_aggregators = [{str(i+1) +'_layer_aggregators':[] for i in range(self.m)} for k in range(self.times)]
        self.list_layer_aggregators.append({str(i+1) +'_layer_aggregators':[] for i in range(self.r)})
        self.list_layer_aggregators[0]['1_layer_aggregators'].append(self.positive_base_aggregator)
        self.list_layer_aggregators[0]['1_layer_aggregators'].append(self.negative_base_aggregator)
        for k in range(self.times):
            for i in range(1, self.m+1):
                if k != 0 and i == 1:
                    [self.list_layer_aggregators[k][str(i) +'_layer_aggregators'].append(SignedSAGEConvolution_tm_1(self.neurons[i-2]*3*(2**(self.m-1)),
                                                                               self.neurons[i-1]).to(self.device)) for j in range(2**(i))]
                    self.list_layer_aggregators[k][str(i) +'_layer_aggregators'] = ListModule(*self.list_layer_aggregators[k][str(i) +'_layer_aggregators'])
                if k != 0 and i != 1:
                    [self.list_layer_aggregators[k][str(i) +'_layer_aggregators'].append(SignedSAGEConvolutionBase(2*self.neurons[i-2],
                                                                               self.neurons[i-1]).to(self.device)) for j in range(2**(i))]
                    self.list_layer_aggregators[k][str(i) +'_layer_aggregators'] = ListModule(*self.list_layer_aggregators[k][str(i) +'_layer_aggregators'])
                if k == 0 and i != 1:
                    [self.list_layer_aggregators[k][str(i) + '_layer_aggregators'].append(
                        SignedSAGEConvolutionBase(2 * self.neurons[i - 2],
                                                  self.neurons[i - 1]).to(self.device)) for j in range(2 ** (i))]
                    self.list_layer_aggregators[k][str(i) + '_layer_aggregators'] = ListModule(
                        *self.list_layer_aggregators[k][str(i) + '_layer_aggregators'])
        for i in range (1,self.r+1):
            if i == 1:
                [self.list_layer_aggregators[self.times][str(i) + '_layer_aggregators'].append(SignedSAGEConvolution_tm_1(2**(self.m-1) * self.neurons[i-2]*3,
                                              self.neurons[i - 1]).to(self.device)) for j in range(2 ** (i))]
                self.list_layer_aggregators[self.times][str(i) + '_layer_aggregators'] = ListModule(*self.list_layer_aggregators[self.times][str(i) + '_layer_aggregators'])
            else:
                [self.list_layer_aggregators[self.times][str(i) + '_layer_aggregators'].append(SignedSAGEConvolutionBase(2 * self.neurons[i - 2],
                                              self.neurons[i - 1]).to(self.device)) for j in range(2 ** (i))]
                self.list_layer_aggregators[self.times][str(i) + '_layer_aggregators'] = ListModule(*self.list_layer_aggregators[self.times][str(i) + '_layer_aggregators'])
        if self.r == 0:
            self.regression_weights = Parameter(torch.Tensor(2**(self.m+1) * self.neurons[-1], 3))  # 4*32=128; edge_label参数
        else:
            self.regression_weights = Parameter(torch.Tensor(2 ** (self.r+1) * self.neurons[-1], 3))
    ...
    def forward(self, positive_edges, negative_edges, target):  # 训练集正边和负边
        """
        ...
        self.h = []
        self.h.append({})
        self.h[0][torch.tanh(self.positive_base_aggregator(self.X, positive_edges))] = 1
        self.h[0][torch.tanh(self.negative_base_aggregator(self.X, negative_edges))] = -1
        for k in range(self.times+1):
            for i in range(len(self.list_layer_aggregators[k])):
                if i != 0:
                    self.h.append({})
                    num = 0
                    for j in list(self.h[-2].keys()):
                        if self.h[-2][j] == 1:
                            self.h[-1][torch.tanh(self.list_layer_aggregators[k][str(i + 1) + '_layer_aggregators'][num](j, positive_edges))] = 1
                            num = num + 1
                            self.h[-1][torch.tanh(self.list_layer_aggregators[k][str(i + 1) + '_layer_aggregators'][num](j, negative_edges))] = -1
                            num = num + 1
                        else:
                            self.h[-1][torch.tanh(self.list_layer_aggregators[k][str(i+1) + '_layer_aggregators'][num](j, positive_edges))] = -1
                            num = num + 1
                ...
                if i == 0 and k != 0:
                    self.h.append({})
                    bf = torch.cat([l for l in self.h[-2].keys() if self.h[-2][l] == 1], 1)
                    uf = torch.cat([l for l in self.h[-2].keys() if self.h[-2][l] == -1], 1)
                    num = 0
                    self.h[-1][torch.tanh(self.list_layer_aggregators[k][str(i + 1) + '_layer_aggregators'][num](bf, uf, positive_edges, negative_edges))] = 1
                    num = num + 1
                    self.h[-1][torch.tanh(self.list_layer_aggregators[k][str(i + 1) + '_layer_aggregators'][num](uf, bf, positive_edges, negative_edges))] = -1
        self.z = torch.cat(list(self.h[-1].keys()), 1)
...
class SignedGCNTrainer(object):
    ...
    def score_model(self, epoch):
        ...
        predictions = probability_scores[:, 0]/probability_scores[:, 0:2].sum(1)  # 将分数转换为概率
        ...
        print('epoch:',epoch+1,'auc:', auc,'f1:', f1)
        self.logs["performance"].append([epoch+1, auc, f1])
        return [epoch+1, loss.item(), auc]
    ...
    def create_and_train_model(self):
        ...
        for epoch in self.epochs:
            ...
            if self.args.test_size > 0:
                epoch_loss_auc = self.score_model(epoch)
            res_plot.append(epoch_loss_auc)
        pd.DataFrame(res_plot).to_csv('Alpha-'+str(len(self.args.layers))+'-epoch_loss_auc.csv')
